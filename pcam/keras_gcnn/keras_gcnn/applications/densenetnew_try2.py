...
def GDenseNetFCN(input_shape, nb_dense_block=5, growth_rate=16, nb_layers_per_block=4, reduction=0.0, dropout_rate=0.5,
...
def __transition_block(ip, nb_filter, padding, bn_momentum, use_g_bn, mc_bn, block_prefix=None, compression=1.0,
    ...
    with K.name_scope('Transition'):
        ...
        x = __BatchNorm(use_g_bn, conv_group, use_gcnn, momentum=bn_momentum, epsilon=1.1e-5, axis=concat_axis,
                        name=name_or_none(block_prefix, '_bn'))(x, training=mc_bn)
        x = Activation('relu')(x)
        x = __Conv2D(int(nb_filter * compression), kernel_size = (3,3), strides = (1, 1), kernel_initializer='he_normal', padding='valid',
    ...
    with K.name_scope('DenseNet'):
        ...
        if type(nb_layers_per_block) is list or type(nb_layers_per_block) is tuple:
            ...
            if nb_layers_per_block == -1:
                ...
                assert (depth - 4) % 5 == 0, 'Depth must be 5 N + 4 if nb_layers_per_block == -1'
                count = int((depth - 4) / 5)
