...
from keras_gcnn.keras_gcnn.layers import GConv2D, GBatchNorm
from keras_gcnn.keras_gcnn.layers.pooling import GroupPool
...
def GDenseNet(mc_dropout, padding, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
              ...
              include_top=True, weights=None, input_tensor=None, pooling=None, classes=10, activation='sigmoid',
              input_shape=None, depth=40, bn_momentum=0.95, use_gcnn=False, conv_group=None, depth_multiplier=1,
...
def GDenseNetFCN(input_shape, nb_dense_block=5, growth_rate=16, nb_layers_per_block=4, reduction=0.0, dropout_rate=0.0,
                 ...
                 bn_momentum=0.95, use_g_bn=True, use_gcnn=False, conv_group=None, mc_bn=None):
    ...
    with K.name_scope('DenseNet'):
        ...
        if type(nb_layers_per_block) is list or type(nb_layers_per_block) is tuple:
            ...
            if nb_layers_per_block == -1:
                # 这里把%3变成了%5，因为有5个dense block
                assert (depth - 4) % int(nb_dense_block) == 0, 'Depth must be x N + 4 if nb_layers_per_block == -1'
                count = int((depth - 4) / int(nb_dense_block))
        ...
        if subsample_initial_block:
            initial_kernel = (3, 3) #修改
            initial_strides = (1, 1)  #修改
        else:
            initial_kernel = (3, 3) #修改
            initial_strides = (1, 1)
        ...
        if use_gcnn:
            ...
            '''
            print ('x = GConv2D 1 shape:',x.shape)
            x = __BatchNorm(use_g_bn, conv_group, use_gcnn, momentum=bn_momentum, epsilon=1.1e-5, axis=concat_axis,
                            name='initial_bn')(x, training=mc_bn)
            print ('x BatchNorm shape:',x.shape)
            x = Activation('relu')(x)
            print ('x Activation shape:',x.shape)
            x = GConv2D(int(round(nb_filter * depth_multiplier)), initial_kernel, kernel_initializer='he_normal',
                        padding='valid', name='initial_Gconv2D',
                        strides=initial_strides, use_bias=False, kernel_regularizer=l2(weight_decay),
                        h_input=conv_group, h_output=conv_group)(x)
            print ('x = GConv2D 2 shape:',x.shape)
            '''
        ...
        if subsample_initial_block:
            x = __BatchNorm(use_g_bn, conv_group, use_gcnn, momentum=bn_momentum, epsilon=1.1e-5, axis=concat_axis,
                            name='initial_bn')(x, training=mc_bn)
            x = Activation('relu')(x)
            x = MaxPooling2D((3, 3), strides=(1, 1), padding=padding)(x)
        ...
        if include_top:
            ...
            if pooling == 'avg':
                x = GlobalAveragePooling2D(stride=2)(x)
            if pooling == 'max':
                x = GlobalMaxPooling2D(stride=2)(x)
